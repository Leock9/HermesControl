...
public class CanceledWorker : BackgroundService
{
    private readonly ILogger<CanceledWorker> _logger;
    private readonly IOrderQueue _orderQueue;
    private readonly IPaymentGateway _paymentService;
    private readonly AsyncCircuitBreakerPolicy _circuitBreakerPolicy;
    private readonly IOrderUseCase _orderUseCase;

    public CanceledWorker(ILogger<CanceledWorker> logger, IOrderQueue orderQueue, IPaymentGateway paymentService, IOrderUseCase orderUseCase)
    {
        _logger = logger;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            var order = await _orderQueue.ConsumeAsync(Status.Canceled.ToString());
            ...
            if (order is not null)
            {
                ...
                try
                {
                    ...
                    await _circuitBreakerPolicy.ExecuteAsync(async () =>
                    {
                        ...
                        await _orderUseCase.UpdateStatusOrderAsync(new UpdateOrderStatusRequest(order.Id, (int)Status.Canceled));
